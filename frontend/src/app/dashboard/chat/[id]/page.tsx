...
function parseAssistantPayload(content: string) {
  // 兼容旧格式：没有 envelope 的历史消息按普通 Markdown 展示。
  if (!content.includes("__LLM_RESPONSE__")) {
    return { content };
  }

  const [base64Part, ...responseParts] = content.split("__LLM_RESPONSE__");
  const responseText = responseParts.join("__LLM_RESPONSE__");
  const envelope = base64Part
    ? (JSON.parse(decodeBase64Utf8(base64Part.trim())) as {
        context?: Array<{
          page_content: string;
          metadata: Record<string, any>;
        }>;
        trace?: Record<string, any>;
        answer_policy?: Record<string, any>;
      })
    : {};

  const citations: Citation[] =
    envelope.context?.map((citation, index) => ({
      id: index + 1,
  ...
  return {
    ...
    answerPolicy: envelope.answer_policy,
  };
}

function decodeBase64Utf8(value: string) {
  // 后端 envelope 用 UTF-8 JSON 编码，浏览器 atob 需要转回字节再解码。
    ...
    return messages.map((message) => {
      ...
      try {
        ...
        return {
          ...message,
          ...parsed,
          content: markdownParse(parsed.content || ""),
        };
      } catch (e) {
        console.error("Failed to process message:", e);
        return message;
            ...
            message.role === "assistant" ? (
              ...
              >
                ...
                <div className="max-w-[80%] rounded-lg px-4 py-2 text-accent-foreground">
                  ...
                  <RetrievalTracePanel
                    trace={message.trace}
                    answerPolicy={message.answerPolicy}
                  />
                </div>
              </div>
            ) : (
