...
export function DocumentUploadSteps({
  ...
  const handlePreview = async () => {
    ...
    const selectedFile = files.find(
      (f) =>
        f.documentId === documentId || f.uploadId === documentId
    );
    ...
    try {
      ...
      const preview = data[documentId] as PreviewResponse | undefined;
      if (preview?.suggested_metadata) {
        setConfirmedMetadata((prev) => ({
          ...prev,
          [documentId]: {
            ...(prev[documentId] || {}),
            ...preview.suggested_metadata,
          },
        }));
      }
    ...
    const resultsToProcess =
      ...
      files
        ...
        .map((f) => ({
          ...
          suggested_metadata: f.uploadId
            ? uploadedDocuments[f.uploadId]?.suggested_metadata
            : undefined,
          confirmed_metadata: f.uploadId
            ? confirmedMetadata[f.uploadId]
            : undefined,
        }));
  ...
  // Update one user-confirmed metadata field for the selected upload.
  const handleMetadataChange = (documentId: number, key: string, value: string) => {
    setConfirmedMetadata((prev) => ({
      ...prev,
      [documentId]: {
        ...(prev[documentId] || {}),
                  ...
                  <div className="rounded-md border p-4">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {uploadedDocuments[selectedDocumentId].total_parent_chunks || 0} parent chunks
                      </span>
                    </div>
                    <div className="grid gap-3 md:grid-cols-2">
                      {[
                        ...
                        <div key={key} className="space-y-1">
                          ...
                          <Input
                            id={`metadata-${key}`}
                            value={confirmedMetadata[selectedDocumentId]?.[key] || ""}
                            onChange={(event) =>
                      ...
                      >
                        ...
                        {task &&
                          (task.status === "pending" ||
                            ...
                            <Progress
                              value={task.progress ?? (task.status === "processing" ? 50 : 25)}
                              className="w-full"
                            />
                          )}
                      </div>
