...
from app.models.knowledge import KnowledgeBase, Document, ProcessingTask, DocumentChunk, DocumentUpload, DocumentParentChunk
...
async def preview_kb_documents(
    ...
    for doc_id in preview_request.document_ids:
        ...
        if upload:
            upload.metadata_suggestion = preview.suggested_metadata
            db.add(upload)
            db.commit()
...
async def process_kb_documents(
    ...
    # Persist metadata confirmed on the upload wizard so background ingestion can copy it to all layers.
    result_by_upload_id = {
        result.get("upload_id"): result
        for result in upload_results
        if result.get("upload_id")
    }
    for upload_id, result in result_by_upload_id.items():
        upload = uploads_dict.get(upload_id)
        if not upload:
            continue
        if result.get("suggested_metadata"):
            upload.metadata_suggestion = result.get("suggested_metadata")
        if result.get("confirmed_metadata"):
            upload.confirmed_metadata = result.get("confirmed_metadata")
    db.commit()
...
@router.post("/{kb_id}/documents/metadata/suggest")
async def suggest_document_metadata(
    kb_id: int,
    request: MetadataSuggestRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Suggest metadata for an uploaded or processed document."""
    kb = db.query(KnowledgeBase).filter(
        KnowledgeBase.id == kb_id,
        KnowledgeBase.user_id == current_user.id
    ).first()
    ...
    if request.upload_id:
        upload = db.query(DocumentUpload).filter(
            DocumentUpload.id == request.upload_id,
            DocumentUpload.knowledge_base_id == kb_id
        ).first()
        if upload:
            file_name = upload.file_name
    ...
    if not file_name:
        raise HTTPException(status_code=400, detail="file_name, upload_id, or document_id is required")

    suggestion = MetadataSuggestionService.suggest(file_name, sample_text)
    if upload:
        upload.metadata_suggestion = suggestion
        db.add(upload)
    if document:
        document.metadata_suggestion = suggestion
        db.add(document)
    db.commit()
    return suggestion

@router.post("/{kb_id}/documents/{doc_id}/metadata/confirm", response_model=DocumentResponse)
async def confirm_document_metadata(
    kb_id: int,
    doc_id: int,
    metadata: DocumentMetadataConfirm,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Document:
    """Confirm document metadata and propagate scalar fields to stored chunks."""
    document = db.query(Document).join(KnowledgeBase).filter(
        Document.id == doc_id,
        Document.knowledge_base_id == kb_id,
        KnowledgeBase.user_id == current_user.id
    ...
    confirmed = metadata.dict(exclude_unset=True)
    final_metadata = merge_confirmed_metadata(document.metadata_suggestion, confirmed)
    ...
    document.metadata_confirmed = True

    # Existing parent/child rows are updated so scalar filters and citations stay consistent.
    parents = db.query(DocumentParentChunk).filter(DocumentParentChunk.document_id == doc_id).all()
    for parent in parents:
        parent.doc_type = document.doc_type
        parent.department = document.department
        parent.effective_date = document.effective_date
        parent.parent_metadata = {**(parent.parent_metadata or {}), **final_metadata}

    chunks = db.query(DocumentChunk).filter(DocumentChunk.document_id == doc_id).all()
    for chunk in chunks:
        chunk.doc_type = document.doc_type
        chunk.department = document.department
        chunk.effective_date = document.effective_date
        chunk.chunk_metadata = {**(chunk.chunk_metadata or {}), **final_metadata}

    db.commit()
    # metadata 变化会影响 Router/profile 和权限展示，确认后同步刷新 KB profile。
    KnowledgeBaseProfileService.build_profile(db, kb_id)
    db.refresh(document)
    return document

@router.post("/{kb_id}/profile/refresh")
async def refresh_knowledge_base_profile(
    kb_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """Refresh and return the routing profile for one knowledge base."""
    kb = db.query(KnowledgeBase).filter(
        KnowledgeBase.id == kb_id,
        KnowledgeBase.user_id == current_user.id
    ).first()
...
async def test_retrieval(
    ...
    try:
        ...
        retrieval_config = RetrievalConfig(
            ...
            kb_router_enabled=request.kb_router_enabled,
            top_k=request.top_k,
            child_candidates_k=max(request.top_k * 4, settings.RETRIEVAL_CHILD_CANDIDATES_K),
            filters=request.filters or {},
        )
        retrieval_result = ParentContextRetriever(db).retrieve(
            query=request.query,
        ...
        for index, doc in enumerate(retrieval_result.documents, start=1):
            response.append({
                ...
                "score": doc.metadata.get("rerank_score") or doc.metadata.get("dense_score"),
                "rank": index,
            })
